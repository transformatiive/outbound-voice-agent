...
1. `POST /api/outbound` → Telnyx `POST /v2/calls` with bidirectional media streaming. When `tts_provider=openai`, the OpenAI Realtime WebSocket is opened **before** dial so the session is already warm at answer.
...
- `tts_provider=elevenlabs` → real Benedita (`ELEVENLABS_VOICE_ID`, default `NkpT2jezLnCDRKHkWiX`) + `eleven_v3` PCMU. Grok Voice Live still does STT and dialogue. **Never** fall back to ara when `elevenlabs` is requested: if the key is missing, `POST /api/outbound` returns **503 `elevenlabs_not_configured`** and does not dial.
...
| Provider | This PR |
| --- | --- |
| **openai** | Realtime WebSocket pre-connects at `POST /api/outbound` (before Telnyx answer). Greeting PCMU is generated out-of-band while muted and flushed on unlock. Barge-in sends Telnyx `clear` + `response.cancel` + `output_audio_buffer.clear`. Phone `server_vad` silence **160ms**, `interrupt_response` on. |
| **grok** | Same latency bar. 160ms VAD, immediate audio forward, barge-in `clear` + `response.cancel`. Session currently opens when Telnyx `/media-stream` connects (after answer); pre-warm is not in this PR. |
| **elevenlabs** | Same latency bar. waitForCallee mute + barge-in abort of in-flight TTS. Greeting TTS starts at `speakGreeting` (unlock), not pre-cached; pre-warm TTS is not in this PR. |
...
- **`tts_provider=elevenlabs`:** Telnyx inbound PCMU still goes to Grok for STT; outbound Telnyx `media` is ElevenLabs μ-law 8 kHz (`ulaw_8000`, with `pcm_8000` / `pcm_16000` resample fallback).
- **`tts_provider=openai`:** Telnyx ↔ OpenAI Realtime `audio/pcmu` (G.711 μ-law 8 kHz). No resample. Same barge-in: Telnyx `clear` plus OpenAI `response.cancel`.

**8 kHz phone caveat:** PSTN PCMU is narrowband. The ChatGPT app uses 24 kHz; on the phone the voice has less “air” and sibilance. OpenAI `audio/pcmu` matches Telnyx so we do not upsample. pt-PT is locked in session instructions (never Brazilian vocabulary). Expect Realtime to sound slightly less rich than the app, with the same turn-taking target (160ms end-of-turn silence, interrupt on callee speech).

Caller barge-in sends Telnyx `clear`, cancels in-flight ElevenLabs playback when that path is selected, **and** `response.cancel` so playback and the in-flight model turn both stop mid-sentence (except while `end_call` hangup is waiting for goodbye playout). Grok and OpenAI `server_vad` use a short end-of-turn silence (`GROK_VAD_SILENCE_MS`, default **160ms**) so replies start as soon as the callee finishes. Leave this at 160 — dropping it further can chop barge-in. `turn_detection.interrupt_response` is enabled. The Telnyx bridge forwards inbound audio immediately (no extra debounce after `speech_stopped`). Optional `GROK_VOICE_SPEED` maps to xAI `audio.output.speed` (0.7–1.5, default **1.05** — leave this as-is; liveliness comes from the prompt, not speed). Voice id stays `ara` unless `GROK_VOICE` is set. OpenAI voice defaults to `coral` (`OPENAI_VOICE` / `openai_voice`); it cannot change after the first audio of the session. xAI Speech-to-Speech prompting forbids stage directions; TTS `[pause]` tags are **not** injected into `force_message` or the realtime stream.
