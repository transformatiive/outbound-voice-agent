...
| **elevenlabs** | ElevenLabs HTTP prefetch at **dial** into a PCMU cache; Grok WS also opens at dial. Unlock is cache playback (no TTS HTTP on the critical path). `optimize_streaming_latency` default **3** is sent only for flash / turbo / multilingual_v2 — **omitted for `eleven_v3`** (that model 400s if the param is present). If prefetch still 400s on the param, unlock retries TTS without it (never ara). Barge-in abort of in-flight TTS + `response.cancel`. Same 130ms VAD (`ELEVENLABS_VAD_SILENCE_MS` optional override). |
