...
Outbound-only voice agent over **Telnyx Call Control** (Alfaseguros). Four interchangeable providers on the same dial API: **Grok Voice Live 2**, **ElevenLabs** (hybrid TTS), **OpenAI Realtime**, and **GPT-Live-1** (ChatGPT Voice, `tts_provider=gpt-live` / `chatgpt-live-1`).
...
3. Provider audio:
   - `tts_provider=grok` (default): this app opens `wss://api.x.ai/v1/realtime` (Grok Voice Live 2, voice from request `grok_voice` or env `GROK_VOICE`, default `ara`) for **STT + dialogue + voice**. Grok audio is forwarded as Telnyx `media` PCMU frames.
   ...
   - `tts_provider=gpt-live`: GPT-Live-1 (`OPENAI_LIVE_MODEL`, default `gpt-live-1`, voice `marin`) owns **full-duplex speech-to-speech** on `wss://api.openai.com/v1/live/sessions`. Audio is Telnyx PCMU (`audio/pcmu` 8 kHz). The live prompt is European Portuguese (`pt-PT`) — never Brazilian; default voice is **`marin`**, not `bossa`/`tempo` (those are Brazilian Portuguese voices). Tools (`end_call`, `send_dtmf`) go through Responses delegation (`OPENAI_LIVE_DELEGATE_MODEL`, default `gpt-5.6-terra`). Same `OPENAI_API_KEY`. Missing key → **503 `openai_not_configured`**; Live session not ready → **503 `gpt_live_session_failed`**. Never falls back to Grok.
...
`tts_provider` is optional: `grok` | `elevenlabs` | `openai` | `gpt-live` (default `grok`). Aliases for GPT-Live: `gpt-live-1` | `chatgpt-live` | `chatgpt-live-1`. The four are **interchangeable on the same API/call**. Same Telnyx dial, same `persona` / `objective` / roles / `pt-PT` fields — only the voice backend changes.
